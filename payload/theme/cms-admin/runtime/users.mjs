  ...
  return {
    ...
    render(){
      ...
      const userList=h('div',{class:'uc-list'},this.items.map(user=>{
        ...
        return h('article',{class:['uc-user',this.selected&&idOf(this.selected)===idOf(user)?'is-selected':''],key:idOf(user),role:'button',tabindex:0,'aria-pressed':this.selected&&idOf(this.selected)===idOf(user),onClick:()=>{this.selected=user},onKeydown:e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();this.selected=user}}},[
          h('div',{class:'uc-user__main'},[
            h('div',{class:'uc-avatar'},esc((user.name||user.username||'?')).slice(0,1).toUpperCase()),
            h('div',{},[h('strong',{},user.name||[user.fname,user.lname].filter(Boolean).join(' ')||user.username||`#${idOf(user)}`),h('small',{},user.email||user.mobile||user.username||`#${idOf(user)}`)])
          ]),
      ...
      return h(LPage,{title:tr('routes.users.title'),description:tr('routes.users.lead')}, {default:()=>h('div',{class:'uc-page'},[
        ...
        this.view==='roles'?roleCenter:h('div',{class:'uc-stack'},[
          ...
          h(LPanel,{title:tr('users_page.list')}, {default:()=>h('div',{class:'uc-stack'},[h('div',{class:'uc-filters'},[h('label',{class:'uc-field'},[h('span',{},tr('users_page.search')),h('input',{type:'search',value:this.filters.q,placeholder:tr('users_page.search_placeholder'),onInput:e=>{this.filters.q=e.target.value;this.search()}})]),select(this.filters.status,v=>{this.filters.status=v;this.load(true)},statusOptions,tr('users_page.status')),select(this.filters.role,v=>{this.filters.role=v;this.load(true)},roleOptions,'Role')]),this.loading?h('p',{},tr('users_page.loading')):this.items.length?userList:h('p',{class:'uc-muted'},tr('users_page.not_found')),h('div',{class:'uc-pager'},[h(LButton,{label:tr('users_page.previous'),severity:'secondary',disabled:this.pagination.offset<=0||this.loading,onClick:this.prev}),h('small',{},tr('users_page.count','',{count:this.pagination.total||0,range:`${this.pagination.offset+1}-${Math.min(this.pagination.offset+this.pagination.returned,this.pagination.total||0)}`})),h(LButton,{label:tr('users_page.next'),severity:'secondary',disabled:!this.pagination.has_more||this.loading,onClick:this.next})])])}),
          detail,
        ])
      ])})
    }
