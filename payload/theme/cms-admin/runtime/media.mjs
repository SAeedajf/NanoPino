import { api, ui, input, textarea, select, message, confirmFa, tr, locale, can } from './common.mjs'
...
export function createComponent(host){
  ...
  return {
    ...
    data(){return{
      ...
      uploads:[], uploading:false, drag:false, permissions:{upload:can('media.upload'),update:can('media.update'),delete:can('media.delete')},
    ...
    methods:{
      ...
      async saveMetadata(){
        if(!this.permissions.update){this.error=tr('state.denied_message');return}
      ...
      async remove(){
        if(!this.permissions.delete){this.error=tr('state.denied_message');return}
      ...
      addFiles(files){
        if(!this.permissions.upload){this.error=tr('state.denied_message');return}
    ...
    render(){
      ...
      const upload=h(LPanel,{title:tr('media_page.secure_upload')},{default:()=>h('div',{style:{display:'grid',gap:'10px'}},[
        h('input',{ref:'fileInput',type:'file',multiple:true,'aria-label':tr('a11y.media_upload',tr('media_page.upload_input')),style:{display:'none'},accept:(this.policy.allowed_extensions||[]).map(v=>'.'+v).join(','),onChange:this.onFileChange}),
          ...
          h('strong',{},tr('media_page.drop_files')),h('small',{},tr('media_page.max_policy','',{max:this.policy.max_mb||20,extensions:(this.policy.allowed_extensions||[]).join(', ')||tr('media_page.server_policy')})),h(LButton,{label:this.uploading?tr('media_page.uploading'):tr('media_page.select_files'),disabled:this.uploading||!this.permissions.upload,onClick:this.pickFiles})
        ]),
      ...
      const detail=this.detail?h(LPanel,{title:tr('media_page.details')},{default:()=>h('div',{class:'cms-media-detail'},[
        ...
        h('div',{class:'cms-media-actions'},[h(LButton,{label:this.saving?tr('media_page.saving'):tr('media_page.save'),disabled:this.saving||!this.permissions.update,onClick:this.saveMetadata}),this.detail.url?h(LButton,{label:tr('media_page.copy_url'),severity:'secondary',onClick:this.copyUrl}):null,h(LButton,{label:tr('media_page.close'),severity:'secondary',onClick:this.closeDetail}),this.permissions.delete?h(LButton,{label:this.deleting?tr('media_page.deleting'):tr('media_page.delete'),severity:'danger',disabled:this.deleting,onClick:this.remove}):null]),
