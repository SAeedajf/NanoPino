...
const FALLBACK_PLATFORM={id:'nanoshell',name:'NanoShell',contract:'nanoshell-platform-v1',version:1,standalone:true,product:'NanoPino',capabilities:['auth.rbac','content.editorial','storage.native','theme.native','builder.native','extensions.signed','recovery.safe-mode'],compatibility:{contract:'nanoshell-platform-v1',minimum_version:1,host_bindings:'pinoox-native'},runtime:{execution:'native-only',document:'block-document-v1',api:'cms-api-v1',identity_source:'nanoshell-platform-contract'},source_adapters:['static-theme-import','static-asset-intake']}
let cachedPlatformInput,cachedPlatformValue
export function platform(){const value=boot().cmsAdmin?.platform;if(value===cachedPlatformInput&&cachedPlatformValue)return cachedPlatformValue;if(!value||typeof value!=='object'||value.id!==FALLBACK_PLATFORM.id||value.name!==FALLBACK_PLATFORM.name||value.contract!==FALLBACK_PLATFORM.contract||!Number.isInteger(value.version)||value.version!==FALLBACK_PLATFORM.version||value.standalone!==true||value.product!==FALLBACK_PLATFORM.product||!Array.isArray(value.capabilities)||!FALLBACK_PLATFORM.capabilities.every(capability=>value.capabilities.includes(capability))||value.compatibility?.contract!==FALLBACK_PLATFORM.compatibility.contract||!Number.isInteger(value.compatibility?.minimum_version)||value.compatibility.minimum_version<1||value.compatibility.minimum_version>FALLBACK_PLATFORM.version||value.compatibility?.host_bindings!==FALLBACK_PLATFORM.compatibility.host_bindings||value.runtime?.execution!==FALLBACK_PLATFORM.runtime.execution||value.runtime?.api!==FALLBACK_PLATFORM.runtime.api){cachedPlatformInput=value;cachedPlatformValue=FALLBACK_PLATFORM;return cachedPlatformValue}cachedPlatformInput=value;cachedPlatformValue=value;return cachedPlatformValue}
export function platformName(){return String(platform().name||FALLBACK_PLATFORM.name)}
export function platformSupports(capability){const requested=String(capability||'').trim();return Boolean(requested&&platform().capabilities.includes(requested))}
