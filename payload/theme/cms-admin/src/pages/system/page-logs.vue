  ...
  <LPage icon="scroll-text" class="cms-logs-page">
    ...
    <LPanel>
      ...
      <div class="cms-log-list">
        <div v-if="loading" class="cms-empty-state" role="status" aria-live="polite">{{ t('logs_page.loading') }}</div>
        ...
        <div v-if="!loading && !logs.length && !error" class="cms-empty-state">{{ t('logs_page.empty') }}</div>
      </div>
    </LPanel>
...
const logs=ref([]),summary=ref({}),health=ref({}),loading=ref(false),exporting=ref(false),showTechnical=ref(false),pageSize=ref(50),activeMinutes=ref(15),error=ref('')
...
async function load(){loading.value=true;error.value='';try{const r=await systemApi.logs({limit:pageSize.value,active_window:activeMinutes.value*60,...filters});logs.value=r.data?.logs||[];summary.value=r.data?.summary||{};health.value=r.data?.health||{}}catch(e){error.value=e.message}finally{loading.value=false}}
...
async function exportBundle(){exporting.value=true;error.value='';try{const r=await systemApi.supportBundle();downloadJson(`nanopino-support-${Date.now()}.json`,r.data?.bundle||r.data)}catch(e){error.value=e.message}finally{exporting.value=false}}
