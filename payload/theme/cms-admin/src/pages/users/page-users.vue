  ...
  <LPage icon="users">
    ...
    <template v-if="view==='users'">
      ...
      <div class="cms-control-filters cms-user-filters">
        <input v-model="filters.q" type="search" :placeholder="t('users_page.search_placeholder')" @input="debouncedLoad">
        <select v-model="filters.status" @change="loadUsers(true)"><option value="">{{ t('users_page.all_statuses') }}</option><option v-for="s in statuses" :key="s" :value="s">{{ statusLabel(s) }}</option></select>
        <select v-model="filters.role" @change="loadUsers(true)"><option value="">{{ t('users_page.all_roles') }}</option><option v-for="role in roles" :key="role.key" :value="role.key">{{ role.name||role.key }}</option></select>
      </div>
        ...
        <LPanel flush bare>
          <CmsPageState :state="loading ? 'loading' : users.length ? 'ready' : 'empty'" empty-icon="user-round-x" :empty-title="t('users_page.empty')">
            <div class="cms-user-list">
              <article v-for="user in users" :key="user.id" class="cms-user-row" :class="{'is-selected': selectedId===user.id}" @click="selectUser(user)">
                <div class="cms-user-row__identity"><span class="cms-user-avatar">{{ initial(user) }}</span><div><strong>{{ user.name || user.username || `#${user.id}` }}</strong><small>{{ user.email || user.mobile || user.username || `#${user.id}` }}</small></div></div>
