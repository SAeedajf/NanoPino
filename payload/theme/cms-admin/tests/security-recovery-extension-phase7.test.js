...
test('Phase 7 extension operations show progress and cannot overlap', () => {
  ...
  assert.match(page, /if\(busy\.value\|\|installing\.value\|\|inspecting\.value\)return/)
  assert.match(page, /loading:busy\.value===props\.item\.id/)
...
test('Phase 7 runtime fallback keeps recovery and extension actions busy-safe', () => {
  ...
  assert.match(recovery, /this\.busy = true/)
  assert.match(recovery, /disabled: this\.busy/)
  assert.match(extensions, /this\.busy=true/)
  assert.match(extensions, /disabled:!this\.file\|\|this\.busy/)
})
