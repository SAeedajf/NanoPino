import { api, data, ui, input, textarea, select, label, message, safeJson, confirmFa, tr, locale, navigate } from './common.mjs'

const CSS = `
.cms-builder-page .ppage-header{min-inline-size:0}
@media(max-width:1400px){.cms-builder-page .ppage-header{flex-wrap:wrap}.cms-builder-page .ppage-header__text,.cms-builder-page .ppage-header__actions{flex:1 1 100%;inline-size:100%;max-inline-size:100%;min-inline-size:0}.cms-builder-page .ppage-header__actions .cms-card-actions{inline-size:100%;min-inline-size:0}}
...
function attributeDefinitions(def){const attrs=def?.attributes;return Array.isArray(attrs)?attrs:attrs&&typeof attrs==='object'?Object.values(attrs):[]}
function defaultAttributes(def){const out={};for(const a of attributeDefinitions(def)){if(a.default!==undefined&&a.default!==null)out[a.name]=clone(a.default);else if(a.required)out[a.name]=a.type==='integer'||a.type==='number'?0:a.type==='boolean'?false:''}return out}
...
export function createComponent(host){
  ...
  return {
    ...
    methods:{
      ...
      renderCanvasNodes(nodes,path=[]){return (nodes||[]).map((node,index)=>{const p=[...path,index],def=defFor(this.blocks,node.type),selected=pathKey(p)===pathKey(this.selectedPath);return h('article',{class:`cms-builder-node${selected?' selected':''}${this.dropClass(node.id)}`,key:node.id,role:'button',tabindex:0,'aria-pressed':selected,'aria-label':`${def?.title||node.type} — ${node.id}`,'data-builder-node-id':node.id,'data-builder-drop-path':pathKey(p),onClick:e=>{e.stopPropagation();this.selectPath(p)},onKeydown:e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();e.stopPropagation();this.selectPath(p)}else if((e.key==='ArrowUp'||e.key==='ArrowDown')&&(e.ctrlKey||e.metaKey)){e.preventDefault();e.stopPropagation();this.selectPath(p);this.moveSelected(e.key==='ArrowUp'?-1:1)}}},[h('div',{class:'cms-builder-node-head'},[h('div',{},[h('strong',{},def?.title||node.type),h('small',{style:{display:'block',opacity:.6}},node.id)]),h('div',{style:ui.row},[this.canEdit?h('span',{class:'cms-builder-drag-handle',title:tr('builder_page.drag_hint'),'aria-hidden':'true',onPointerdown:e=>this.dragStart(e,p)},'⋮⋮'):null,h(LBadge,{label:`v${node.version||1}`}),node.children?.length?h(LBadge,{label:tr('builder_page.child_count','',{count:node.children.length}),severity:'secondary'}):null])]),node.attributes?.text?h('p',{style:{margin:0}},String(node.attributes.text).slice(0,180)):node.attributes?.label?h('p',{style:{margin:0}},String(node.attributes.label)):null,node.children?.length?h('div',{class:'cms-builder-node-children'},this.renderCanvasNodes(node.children,p)):null])})},
      renderLayers(nodes,path=[]){return (nodes||[]).map((node,index)=>{const p=[...path,index],selected=pathKey(p)===pathKey(this.selectedPath),def=defFor(this.blocks,node.type);return h('div',{key:node.id},[h('div',{class:`cms-builder-layer${selected?' selected':''}${this.dropClass(node.id)}`,role:'button',tabindex:0,'aria-pressed':selected,'aria-label':`${def?.title||node.type} — ${node.id}`,'data-builder-node-id':node.id,onClick:()=>this.selectPath(p),onKeydown:e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();this.selectPath(p)}else if((e.key==='ArrowUp'||e.key==='ArrowDown')&&(e.ctrlKey||e.metaKey)){e.preventDefault();this.selectPath(p);this.moveSelected(e.key==='ArrowUp'?-1:1)}}},[this.canEdit?h('span',{class:'cms-builder-drag-handle',title:tr('builder_page.drag_hint'),'aria-hidden':'true',onPointerdown:e=>this.dragStart(e,p)},'⋮⋮'):null,h('div',{style:{minWidth:0,flex:1}},[h('strong',{},def?.title||node.type),h('code',{style:{display:'block'}},node.id)])]),node.children?.length?h('div',{class:'cms-builder-layer-indent'},this.renderLayers(node.children,p)):null])})},
    },
    render(){
      ...
      return h(LPage,{title:tr('routes.builder.title'),description:tr('routes.builder.lead')},{default:()=>h('div',{class:'cms-builder-center'},[
        ...
        h('div',{class:'cms-builder-toolbar'},[
          ...
          this.target.type==='content'?h(LButton,{label:tr('builder_page.create_content'),onClick:()=>navigate('/content')}):null,
        ...
        h('div',{class:'cms-builder-statusbar',role:'status','aria-live':'polite','aria-busy':this.busy?'true':'false'},[
          h('div',{},[h('strong',{},this.record?this.targetSummary:tr('builder_page.not_open')),h('small',{},`${this.record?(this.dirty?tr('builder_page.unsaved'):tr('builder_page.synced')):tr('builder_page.open_before_edit_help')} · ${tr('builder_page.drag_hint')}`)]),
          h(LBadge,{label:this.dirty?tr('builder_page.unsaved'):this.record?tr('builder_page.synced'):tr('builder_page.not_open'),severity:this.dirty?'warning':this.record?'success':'secondary'}),
        ]),
        h('div',{style:ui.grid},[
        ...
        !this.record?h(LPanel,{title:tr('builder_page.start_title')},{default:()=>h('div',{class:'cms-builder-onboarding'},[
          ...
          h('div',{class:'cms-builder-actions'},[this.target.type==='content'?h(LButton,{label:tr('builder_page.create_content'),onClick:()=>navigate('/content')}):null,h(LButton,{label:tr('builder_page.open_existing'),severity:'secondary',disabled:this.busy||!this.targetReady,onClick:this.openExistingDocument}),h(LButton,{label:tr('builder_page.create_override'),severity:'secondary',disabled:this.busy||!this.canEdit||!this.targetReady,onClick:this.createDocument})])
        ])}):null,
        ...
        this.previewHtml?h(LPanel,{title:tr('builder_page.preview_title','',{viewport:this.viewportLabel(this.viewport)})},{default:()=>h('iframe',{class:'cms-builder-preview',sandbox:'',srcdoc:this.previewHtml,title:tr('builder_page.preview'),style:{maxWidth:this.viewport==='mobile'?'390px':this.viewport==='tablet'?'768px':'100%',margin:'0 auto'}})}):null,
