  ...
  <LPage icon="panels-top-left" class="cms-builder-page">
    <template #actions>
      <div class="cms-card-actions">
        ...
        <LButton v-if="target.type==='content'" icon="file-plus-2" shape="rounded" :disabled="busy" @click="router.push('/content')">{{ t('builder_page.create_content') }}</LButton>
        <LButton icon="layout-template" variant="outline" shape="rounded" :disabled="busy||!canEdit||!!record" @click="createDocument">{{ t('builder_page.create_override') }}</LButton>
        <LButton v-if="record" icon="x" variant="outline" severity="neutral" shape="rounded" :disabled="busy" @click="closeDocument">{{ t('builder_page.close_document') }}</LButton>
    ...
    <div class="cms-builder-statusbar" role="status" aria-live="polite" :aria-busy="busy ? 'true' : 'false'">
      <div>
        <strong>{{ record ? targetSummary : t('builder_page.not_open') }}</strong>
        <small>{{ record ? (dirty ? t('builder_page.unsaved') : t('builder_page.synced')) : t('builder_page.open_before_edit_help') }} · {{ t('builder_page.drag_hint') }}</small>
      </div>
      <LBadge :severity="dirty ? 'warning' : record ? 'success' : 'secondary'">{{ dirty ? t('builder_page.unsaved') : record ? t('builder_page.synced') : t('builder_page.not_open') }}</LBadge>
    </div>
    ...
    <LPanel><template #header>{{ t('builder_page.target') }}</template>
      <div class="cms-builder-target">
        <label>{{ t('builder_page.type') }}
          <select v-model="target.type" :disabled="!!record" @change="resetTargetKey">
        ...
        <label v-if="target.type==='content'">{{ t('builder_page.content_target') }}
          <div class="cms-builder-target-search">
            <input v-model.trim="contentQuery" type="search" :disabled="!!record" :placeholder="t('builder_page.content_search_placeholder')" @keyup.enter="loadContentTargets">
            <LButton size="sm" variant="outline" :disabled="contentLoading||!!record" @click="loadContentTargets">{{ t('builder_page.search') }}</LButton>
          </div>
          <select v-model="target.key" :disabled="!!record">
        ...
        <label v-else-if="target.type==='template'">{{ t('builder_page.template_target') }}
          <select v-model="target.key" :disabled="!!record"><option v-for="item in templateTargets" :key="item.key" :value="item.key">{{ item.label }}</option></select>
        </label>
        <label v-else-if="target.type==='template_part'">{{ t('builder_page.part_target') }}
          <select v-model="target.key" :disabled="!!record"><option v-for="item in partTargets" :key="item.key" :value="item.key">{{ item.label }}</option></select>
        </label>
        ...
        <label>{{ t('builder_page.language') }}<select v-model="target.locale" :disabled="!!record"><option value="fa">{{ t('builder_page.fa') }}</option><option value="en">{{ t('builder_page.en') }}</option><option value="ar">{{ t('builder_page.ar') }}</option></select></label>
    ...
    <LPanel v-if="!record" class="cms-builder-onboarding">
      ...
      <div class="cms-builder-onboarding__body">
        ...
        <div class="cms-card-actions"><LButton v-if="target.type==='content'" @click="router.push('/content')">{{ t('builder_page.create_content') }}</LButton><LButton variant="outline" :disabled="busy||!targetReady" @click="openExistingDocument">{{ t('builder_page.open_existing') }}</LButton><LButton variant="outline" :disabled="busy||!canEdit||!targetReady" @click="createDocument">{{ t('builder_page.create_override') }}</LButton></div>
      </div>
    </LPanel>

    <div class="cms-builder-shell cms-builder-shell--control">
      <aside class="cms-builder-sidebar cms-builder-sidebar--start is-mobile-active" :aria-label="t('builder_page.block_library')">
        <div class="cms-builder-panel-scroll">
          ...
          <div class="cms-builder-panel-head"><strong>{{ t('builder_page.reusable_blocks') }}</strong><LBadge severity="secondary">{{ globalBlocks.length }}</LBadge></div>
          <div class="cms-stack">
      ...
      <main class="cms-builder-canvas-wrap" :aria-label="t('builder_page.canvas')" :aria-busy="busy ? 'true' : 'false'">
        <div class="cms-builder-canvas-frame" :data-viewport="viewport">
          ...
          <div v-else class="cms-builder-canvas cms-builder-canvas--interactive">
            <BuilderNode v-for="(node,index) in document.blocks" :key="node.id" :node="node" :path="[index]" :selected-path="selectedPath" :start-drag="startDrag" :drop-class="dropClass" @select="selectedPath=$event" @move="move=>{selectedPath=move.path;moveSelected(move.delta)}" />
          </div>
      ...
      <aside class="cms-builder-sidebar cms-builder-sidebar--end is-mobile-active" :aria-label="t('builder_page.inspector')">
        <div class="cms-builder-panel-scroll">
    ...
    <LPanel v-if="previewHtml"><template #header>{{ t('builder_page.preview_title', { viewport }) }}</template><iframe class="cms-builder-preview-frame" :srcdoc="previewHtml" sandbox="" :title="t('builder_page.preview')" :style="{maxWidth:viewport==='mobile'?'390px':viewport==='tablet'?'768px':'100%',margin:'0 auto'}" /></LPanel>
...
<script setup>
import { computed,defineComponent,h,onBeforeUnmount,onMounted,reactive,ref } from 'vue';import { useRoute,useRouter } from 'vue-router';import { LBadge,LButton,LIcon,LPage,LPanel,LStatCard } from '@pinooxhq/luma/ui';import { readAdminBootData } from '../../services/admin-provider.js';import { builderApi,contentApi,globalBlockApi,themeApi } from '../../services/cms-api.js';import { t } from '../../i18n/index.js'
const boot=readAdminBootData(),route=useRoute(),router=useRouter(),target=reactive({site_id:Number(route.query.site_id||1),type:String(route.query.type||'template'),key:String(route.query.key??(String(route.query.type||'template')==='content'?'':String(route.query.type||'template')==='site'?'site':'home')),locale:String(route.query.locale||'fa')}),record=ref(null),document=reactive({version:1,blocks:[]}),busy=ref(false),autosaveInFlight=ref(false),error=ref(''),notice=ref(''),previewHtml=ref(''),revisions=ref([]),globalBlocks=ref([]),patterns=ref([]),reusableName=ref(''),reusableBusy=ref(false),viewport=ref('desktop'),selectedPath=ref(null),blockSearch=ref(''),dirty=ref(false),undoStack=ref([]),redoStack=ref([]),advancedJson=ref(''),timer=ref(null),autosaveDebounce=ref(null),advancedSyncTimer=ref(null),lastAutosaveSignature=ref(''),lastMutationKey=ref(''),lastSnapshotAt=ref(0),contentTargets=ref([]),contentQuery=ref(''),contentLoading=ref(false),drag=reactive({active:false,sourceId:null,overId:null,mode:null,pointerId:null}),viewports=['desktop','tablet','mobile']
...
const templateTargets=computed(()=>Array.from(new Set(boot.fullSiteEditor?.templateKinds||['index','home','page','single','archive','taxonomy','search','404'])).map(key=>({key,label:({index:t('site_editor_page.index'),home:t('site_editor_page.home'),page:t('site_editor_page.page'),single:t('site_editor_page.single'),archive:t('site_editor_page.archive'),taxonomy:t('site_editor_page.taxonomy'),search:t('site_editor_page.search'),'404':'404'})[key]||key})))
const partTargets=computed(()=>Array.from(new Set(boot.fullSiteEditor?.templateParts||['header','footer'])).map(key=>({key,label:key==='header'?t('site_editor_page.header'):key==='footer'?t('site_editor_page.footer'):key})))
...
async function run(fn){busy.value=true;error.value='';try{return await fn()}finally{busy.value=false}}
async function findExistingTarget(){if(!targetReady.value)throw new Error(t('builder_page.target_required'));const listing=await builderApi.list({site_id:target.site_id,type:target.type,locale:target.locale,limit:200,projection:'summary'});const data=listing.data||listing;return (data.items||[]).find(x=>x.target?.key===target.key&&x.target?.type===target.type&&x.target?.locale===target.locale)||null}
async function openExistingDocument(){try{await run(async()=>{const found=await findExistingTarget();if(!found)throw new Error(t('builder_page.no_existing_target'));const read=await builderApi.read(found.id);applyRecord(read.data||read)});notice.value=t('builder_page.existing_opened');await loadRevisions();await loadReusableAssets()}catch(e){error.value=e.message}}
async function createDocument(){if(!canEdit.value){error.value=t('builder_page.edit_denied');return}try{await run(async()=>{const found=await findExistingTarget();if(found)throw new Error(t('builder_page.target_already_exists'));const created=await builderApi.create({target:{...target},document:{version:1,blocks:[]}});applyRecord(created.data||created)});notice.value=t('builder_page.override_created');await loadRevisions();await loadReusableAssets()}catch(e){error.value=e.message}}
// Only acknowledge the submitted snapshot; newer local edits remain dirty.
