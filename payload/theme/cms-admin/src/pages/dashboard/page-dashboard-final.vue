<template>
  <LPage icon="layout-dashboard" header-tone="gradient">
    ...
    <div class="cms-dashboard-main-grid">
      <LPanel class="cms-dashboard-chart-panel"><template #header><div class="cms-panel-heading"><div><strong>{{ t('dashboard.content') }}</strong><small>{{ t('dashboard.control_plane_hint') }}</small></div><LBadge severity="success">{{ t('dashboard.system_health') }}</LBadge></div></template><div class="cms-dashboard-chart" role="img" :aria-label="t('dashboard.summary_aria')"><div class="cms-dashboard-chart__grid"><span v-for="line in 4" :key="line" /></div><div class="cms-dashboard-chart__bars"><span v-for="(value,index) in chartBars" :key="index" class="cms-dashboard-bar" :style="{height: `${value}%`}" :title="`${value}%`" :aria-label="`${chartLabels[index] || index + 1}: ${value}%`"><b /></span></div><div class="cms-dashboard-chart__labels"><span v-for="label in chartLabels" :key="label">{{ label }}</span></div></div><div class="cms-dashboard-chart-footer"><span><i class="cms-dot cms-dot--indigo" />{{ t('dashboard.content') }}</span><span>{{ t('dashboard.monitoring_hint') }}</span></div></LPanel>
...
const boot=readAdminBootData(),router=useRouter(),extensions=ref([]),health=ref([]),recentContent=ref([]),loading=ref(false),error=ref(''),lastUpdated=ref('')
const summary=reactive({content:0,media:0,extensions:0,problems:0}),chartBars=[38,46,42,58,52,67,61,73,65,82,75,88],chartLabels=['۱','۵','۱۰','۱۵','۲۰','۲۵']
...
async function load(){loading.value=true;error.value='';try{const [content,media,extension,healthResponse,logs]=await Promise.all([contentApi.list({limit:5,projection:'list'}),mediaApi.list({limit:1}),extensionApi.list(),systemApi.health(),systemApi.logs({limit:1,active_window:900})]);const c=content.data||{},m=media.data||{},ls=logs.data?.summary||{};extensions.value=extension.data?.items||[];health.value=healthResponse.data?.checks||[];recentContent.value=c.items||[];summary.content=Number(c.pagination?.total||0);summary.media=Number(m.summary?.total||0);summary.extensions=extensions.value.length;summary.problems=problemExtensions.value.length+health.value.filter(item=>['error','fail'].includes(item.status)).length+Number(ls.active_errors||0);lastUpdated.value=new Intl.DateTimeFormat(undefined,{hour:'2-digit',minute:'2-digit'}).format(new Date())}catch(e){error.value=e.message}finally{loading.value=false}}
