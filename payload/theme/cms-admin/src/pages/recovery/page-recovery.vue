  ...
  <LPage icon="shield-check" class="cms-recovery-center">
    ...
    <LPanel>
      ...
      <div class="luma-actions">
        <LButton v-if="safeMode.enabled" variant="outline" severity="danger" shape="rounded" :loading="actionBusy === 'safe-mode-disable'" :disabled="Boolean(actionBusy) || !center.apiBound" @click="requestSafeModeDisable">{{ actionBusy === 'safe-mode-disable' ? t('recovery_page.requesting') : t('recovery_page.disable_request') }}</LButton>
...
async function requestSafeModeDisable() {
  ...
  try { const response = await recoveryApi.disableSafeMode(); safeMode.value = { ...safeMode.value, ...(response.data || response) }; actionMessage.value = t('recovery_page.disable_requested') }
  catch (error) { actionError.value = error?.message || t('recovery_page.action_failed') }
  finally { actionBusy.value = '' }
}
</script>
