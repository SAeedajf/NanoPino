import{api,ui,message,confirmFa,tr,can}from'./common.mjs'
...
export function createComponent(host){
 ...
 return{
  ...
  data(){return{items:[],file:null,stage:null,review:null,mode:null,approved:false,error:'',notice:'',loading:false,installing:false,busy:{id:'',action:''},inspectionRequest:0,capabilities:{install:can('extensions.install'),activate:can('extensions.activate'),deactivate:can('extensions.deactivate'),repair:can('extensions.repair'),uninstall:can('extensions.uninstall')} }},
  computed:{
   ...
   reviewPermissions(){return this.reviewData.permissions?.items||[]},
  },
  mounted(){this.load()},
  methods:{
   ...
   async inspect(){if(!this.file||!this.capabilities.install)return;const request=this.inspectionRequest,file=this.file;this.installing=true;this.error='';try{const form=new FormData();form.append('file',file);const d=await api('/extensions/inspect',{method:'POST',form});if(request!==this.inspectionRequest)return;this.stage=d.stage;this.review={...(d.review||{}),manifest:d.review?.manifest||d.review?.package?.manifest||{}};this.mode=d.mode;this.approved=false}catch(e){if(request===this.inspectionRequest)this.error=e?.message||tr('extensions_page.inspect_failed')}finally{if(request===this.inspectionRequest)this.installing=false}},
   clearReview(){this.inspectionRequest++;this.file=null;this.stage=null;this.review=null;this.mode=null;this.approved=false},
   async execute(){
    if(!this.capabilities.install||this.installing||!this.stage||!this.approved||this.reviewData.decision==='block')return
    ...
    }catch(e){this.error=e?.message||tr('extensions_page.operation_failed')}finally{this.installing=false}
   },
   ...
   typeLabel(v){return({'core-module':tr('extensions_page.type_core_module'),module:tr('extensions_page.type_module'),plugin:tr('extensions_page.type_plugin'),integration:tr('extensions_page.type_integration'),theme:tr('extensions_page.type_theme'),'admin-extension':tr('extensions_page.type_admin_extension'),block:tr('extensions_page.type_block'),'block-package':tr('extensions_page.type_block_package'),driver:tr('extensions_page.type_driver'),'language-pack':tr('extensions_page.type_language_pack')})[v]||v},
   ...
   async action(x,a){
    const id=x.id||x.package;if(!id||!this.capabilities[a]||this.installing||this.busy.id)return
    ...
    try{await api(`/extensions/${encodeURIComponent(id)}${a==='uninstall'?'':`/${a}`}`,{method:a==='uninstall'?'DELETE':'POST',body:a==='uninstall'?null:{}});this.notice=tr('extensions_page.operation_done');await this.load()}catch(e){this.error=e?.message||tr('extensions_page.operation_failed')}finally{this.busy={id:'',action:''}}
   },
   reviewPanel(){
    ...
    return h('div',{class:'cms-extension-review'},[
     ...
     h('div',{class:'cms-extension-review-grid'},[
      ...
      h('article',{},[h('strong',{},tr('extensions_page.permissions')),h(LBadge,{label:this.riskLabel(risk),severity:riskSeverity(risk)}),h('small',{},tr('extensions_page.permission_count','',{count:this.reviewPermissions.length}))]),
     ...
     this.reviewPermissions.length?h('details',{},[h('summary',{},tr('extensions_page.permission_details')),h('div',{style:ui.page},this.reviewPermissions.map(p=>h('div',{style:ui.row},[h('code',{},p.permission),h(LBadge,{label:this.riskLabel(p.risk),severity:riskSeverity(p.risk)}),h('small',{},p.description||'')])))]):null,
     ...
     h('div',{style:ui.row},[h(LButton,{label:this.installing?tr('extensions_page.running'):this.mode==='update'?tr('extensions_page.run_update'):tr('extensions_page.install_package'),disabled:this.installing||!this.approved||decision==='block'||!this.capabilities.install,onClick:this.execute}),h(LButton,{label:tr('extensions_page.cancel'),severity:'secondary',disabled:this.installing,onClick:this.clearReview})])
    ])
   },
   row(x){
    ...
    return h(LPanel,{title:x.name||x.package||id,key:id},{default:()=>h('div',{style:ui.page},[
     ...
     h('div',{style:ui.row},[
      this.isCore(x)&&status==='active'?null:status==='active'&&this.capabilities.deactivate?h(LButton,{label:rowBusy&&this.busy.action==='deactivate'?tr('extensions_page.running'):tr('extensions_page.deactivate'),disabled:Boolean(this.busy.id),onClick:()=>this.action(x,'deactivate')}):status!=='active'&&this.capabilities.activate?h(LButton,{label:rowBusy&&this.busy.action==='activate'?tr('extensions_page.running'):tr('extensions_page.activate'),disabled:Boolean(this.busy.id),onClick:()=>this.action(x,'activate')}):null,
      this.capabilities.repair?h(LButton,{label:rowBusy&&this.busy.action==='repair'?tr('extensions_page.running'):tr('extensions_page.repair'),severity:'secondary',disabled:Boolean(this.busy.id),onClick:()=>this.action(x,'repair')}):null,
      !this.isCore(x)&&this.capabilities.uninstall?h(LButton,{label:rowBusy&&this.busy.action==='uninstall'?tr('extensions_page.running'):tr('extensions_page.delete'),severity:'danger',disabled:Boolean(this.busy.id),onClick:()=>this.action(x,'uninstall')}):null
     ])
    ])})
   }
  },
  render(){return h(LPage,{title:tr('routes.extensions.title'),description:tr('routes.extensions.lead')},{default:()=>h('div',{style:ui.page},[
   ...
   h(LPanel,{title:tr('extensions_page.install_update')},{default:()=>h('div',{style:ui.page},[
    ...
    h(LButton,{label:this.installing?tr('extensions_page.running'):tr('extensions_page.inspect'),disabled:!this.file||this.installing||!this.capabilities.install,onClick:this.inspect}),
    this.reviewPanel()
   ])}),
