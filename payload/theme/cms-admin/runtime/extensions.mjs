...
export function createComponent(host){
 ...
 return{
  ...
  methods:{
   ...
   async inspect(){if(!this.file||!this.capabilities.install||this.installing)return;const request=this.inspectionRequest,file=this.file;this.installing=true;this.error='';try{const form=new FormData();form.append('file',file);const d=await api('/extensions/inspect',{method:'POST',form});if(request!==this.inspectionRequest)return;this.stage=d.stage;this.review=d.review;this.mode=d.mode;this.approved=false}catch(e){if(request===this.inspectionRequest)this.error=e?.message||tr('extensions_page.inspect_failed')}finally{if(request===this.inspectionRequest)this.installing=false}},
